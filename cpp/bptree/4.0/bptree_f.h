...
__tt(index_t, T)
ptrdiff_t
bptree<index_t, T>::alloc(uint32_t type)
{
	switch (type) {
	case BP_T:
		return pool_T.alloc()<<2 | BP_T;
	case BP_INDEX:
		return pool_index.alloc()<<2 | BP_INDEX;
	case BP_NODE:
		return pool_node.alloc()<<2 | BP_NODE;
	}
	return OFF_NULL;
}
...
__tt(index_t, T)
void
bptree<index_t, T>::free(ptrdiff_t off)
{
	switch (off & 0x3) {
	case BP_T:
		pool_T.free(off>>2);
		return;
	case BP_INDEX:
		pool_index.free(off>>2);
		return;
	case BP_NODE:
		pool_node.free(off>>2);
		return;
	}
}
...
__tt(index_t, T)
void*
bptree<index_t, T>::getptr(ptrdiff_t off)
{
	switch (off & 0x3) {
	case BP_T:
		return pool_T.getptr(off>>2);
	case BP_INDEX:
		return pool_index.getptr(off>>2);
	case BP_NODE:
		return pool_node.getptr(off>>2);
	}
	return NULL;
}
