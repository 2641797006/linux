...
#define __error__(fmt, ...)	string_fatal_error(__FILE__, __func__, __LINE__, fmt, __VA_ARGS__)
...
{
	if ( ! s->_size )
		__error("read data from a string which is empty\n", 0);
	return *s->_data;
}
